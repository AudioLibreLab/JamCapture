...
func TestGenerateConfigXR18WithChrome(t *testing.T) {
	...
	if _, ok := root.Configs["xr18_hardware"]; !ok {
		t.Error("expected xr18_hardware profile")
	}
	if _, ok := root.Configs["xr18_chrome"]; ok {
		t.Error("unexpected xr18_chrome profile (per-player profiles generated by initsoftware only)")
	}
...
func TestGenerateConfigScarlettOnly(t *testing.T) {
	...
	if _, ok := root.Configs["2i2_hardware"]; !ok {
		t.Error("expected 2i2_hardware profile")
	}
